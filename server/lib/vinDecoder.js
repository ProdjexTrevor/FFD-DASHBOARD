...
function normalizedResult(raw, dealerTrack = null) {
  ...
  if (!make || !SUPPORTED_MAKES.has(make)) {
    const error = new Error(
      `Unsupported manufacturer${make ? `: ${make}` : ""}. Supported makes: ${[...SUPPORTED_MAKES].join(", ")}`
    );
...
async function decodeVin(value, { refresh = false } = {}) {
...
export { decodeVin, normalizeVin, SUPPORTED_MAKES };
